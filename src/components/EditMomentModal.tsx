...
export default function EditMomentModal({ isOpen, onClose, moment }: EditMomentModalProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      try {
        const optimized = await compressImage(selectedFile, 800, 800, 0.5);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (momentImages.length === 0 && !note.trim()) {
         // If they remove all images and note, delete the moment entirely
      ...
      setErrorStatus('Failed to update moment. Connection might be weak.');
    } finally {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            <div className="flex h-16 items-center justify-between border-b border-natural-border px-8">
              <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-natural-text">Edit Moment</h2>
              <button 
                onClick={onClose}
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Write a detail about this part of the journey..."
                ...
                <button
                  type="submit"
                  disabled={isUploading || isOptimizing || (momentImages.length === 0 && !note.trim())}
