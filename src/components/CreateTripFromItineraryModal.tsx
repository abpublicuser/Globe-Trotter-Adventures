...
interface CreateTripFromItineraryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (tripId: string) => void;
}

export default function CreateTripFromItineraryModal({ isOpen, onClose, onSuccess }: CreateTripFromItineraryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !itineraryText || !auth.currentUser) return;
    // Note: preview is optional for itinerary trips, or we can make it required. Let's make it optional.

    setIsProcessing(true);
    setErrorStatus(null);

    try {
      setProcessingStatus('Parsing your itinerary with AI...');
      // 1. Parse itinerary using backend
      ...
      if (!Array.isArray(parsedDays) || parsedDays.length === 0) {
        throw new Error('No valid days found in the itinerary.');
      }

      setProcessingStatus('Creating your journey...');
      ...
      console.error('Failed to create trip from itinerary:', error);
      const errMessage = error instanceof Error ? error.message : 'Failed to start journey from itinerary. Please try again.';
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            <div className="flex h-16 shrink-0 items-center justify-between border-b border-natural-border px-8">
              <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-natural-text">Magic Itinerary Import</h2>
              <button 
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8">
              <div className="space-y-6">
                <div>
                  <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-natural-muted">Trip Name</label>
                  <input
                    type="text"
                    value={name}
                ...
                <div>
                  <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-natural-muted">Paste Itinerary</label>
                  <textarea
                    value={itineraryText}
                    onChange={(e) => setItineraryText(e.target.value)}
                    placeholder="Paste your day-by-day itinerary notes here. Our AI will automatically convert them into structured placeholders for your journey."
                ...
                <button
                  type="submit"
                  disabled={isProcessing || isOptimizing || !name || !itineraryText}
